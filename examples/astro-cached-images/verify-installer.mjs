...
const temporary = await mkdtemp(path.join(tmpdir(), 'astro-images-'));
const root = path.join(temporary, 'application');
const installed = path.join(here, 'node_modules/astro');
...
try {
  ...
  if (await applyPatch(root, { checkOnly: true }) === 'already patched') {
    const reverse = spawnSync('git', ['-c', 'core.autocrlf=false', 'apply', '-R', path.join(here, 'patches/astro-7.3.2.patch')], {
  ...
  await writeFile(metadataPath, JSON.stringify({ ...JSON.parse(metadata), version: '7.3.3' }));
  await assert.rejects(applyPatch(root), /Expected astro@7.3.2/);
  assert.deepEqual(await readFile(target), original);
  ...
  await writeFile(target, original);
  await assert.rejects(applyPatch(root), /only partly applied/);
  assert.deepEqual(await readFile(target), original);

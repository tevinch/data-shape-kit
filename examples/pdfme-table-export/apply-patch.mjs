...
const patchFile = path.join(here, 'patches/common-6.1.12.patch');
const digest = data => createHash('sha256').update(data).digest('hex');

async function installationState(appRoot) {
  const root = await realpath(appRoot);
  let parent = root;
  for (const part of ['node_modules', '@pdfme', 'common']) {
    parent = path.join(parent, part);
    const info = await lstat(parent);
    if (info.isSymbolicLink() || !info.isDirectory()) {
      throw new Error('Use a regular npm node_modules installation; linked package paths are unsupported.');
    }
  }

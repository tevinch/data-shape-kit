...
const temporary = await mkdtemp(path.join(tmpdir(), 'pdfme-export-'));
const root = path.join(temporary, 'application');
const installed = path.join(here, 'node_modules/@pdfme/common');
const packageRoot = path.join(root, 'node_modules/@pdfme/common');
...
try {
  for (const relative of Object.keys(manifest.files)) {
    ...
    await cp(path.join(installed, relative), path.join(packageRoot, relative));
  }
  await cp(path.join(installed, 'package.json'), metadataPath);
  if (await applyPatch(root, { checkOnly: true }) === 'already patched') {
    const reverse = spawnSync('git', ['-c', 'core.autocrlf=false', 'apply', '-R', path.join(here, 'patches/common-6.1.12.patch')], {
  ...
  await writeFile(metadataPath, JSON.stringify({ ...JSON.parse(metadata), version: '6.1.13' }));
  await assert.rejects(applyPatch(root), /Expected @pdfme\/common@6.1.12/);
  assert.deepEqual(await readFile(target), original);

...
const patchFile = path.join(here, 'patches/lime-elements-40.2.2.patch');
const digest = data => createHash('sha256').update(data).digest('hex');

async function installationState(appRoot) {
  const root = await realpath(appRoot);
  let parent = root;
  for (const part of ['node_modules', '@limetech', 'lime-elements']) {
    parent = path.join(parent, part);
    const info = await lstat(parent);
    if (info.isSymbolicLink() || !info.isDirectory()) {
      throw new Error('Use a regular npm installation; linked package paths are unsupported.');
    }
  }
...
export async function applyPatch(appRoot = process.cwd(), { checkOnly = false } = {}) {
  ...
  return 'all six runtime files patched and verified';
}
